...
For each combination, the pipeline:
  ...
  - Runs the CARS model.
  - VALIDATION: Automatically trains an SVM model on selected features and clean data.
...
def load_hsi_dataset(dataset_name):
    ...
    base_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'HSI_WST_Pipeline', 'dataset')
    wavelengths_dir = base_dir
...
def plot_spectral(title, plot_name, all_classes, X_clean, img_folder):
    ...
    plt.savefig(os.path.join(img_folder, plot_name), bbox_inches='tight', dpi=150)
    plt.close()
...
# ------------------------------
def run_svm_validation(X_clean, y_clean, selected_wavelengths, output_dir, method_name):
    """
    Runs SVM-RBF classification on the selected features (outliers already removed from X_clean).
    """
    print(f"  [Validation] Running SVM-RBF for {method_name}...")
    
    # Filter columns to selected wavelengths
    # Find closest columns in X_clean
    available_cols = X_clean.columns.values.astype(float)
    selected_indices = []
    
    for wt in selected_wavelengths:
        # Assuming wt is a float
        idx = np.argmin(np.abs(available_cols - wt))
        selected_indices.append(idx)
    
    selected_indices = list(set(selected_indices))
    X_sel = X_clean.iloc[:, selected_indices].values
    
    # Align target y (MultiIndex level 'Class')
    y_sel = X_clean.index.get_level_values('Class').values.astype(int)
    
    n_feats = len(selected_indices)
    print(f"  [Validation] Selected {n_feats} features.")
    
    if n_feats == 0:
        print("  [Validation] Warning: 0 features selected. Aboard validation.")
        return

    # Train/Test Split (Stratified 30%)
    X_train, X_test, y_train, y_test = train_test_split(
        X_sel, y_sel, test_size=0.3, stratify=y_sel, random_state=42
    )
    ...
    # Save Metrics
    metrics_path = os.path.join(output_dir, f'metrics_{n_feats}.txt')
    ...
    plt.title(f'Confusion Matrix ({method_name} - {n_feats} feats)')
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'cm_{n_feats}.jpg'), dpi=150)
    plt.close()
...
def run_pipeline(n_components, preopocessing_ops, dataset_name, plots_only=False, skip_to=None):
    ...
    img_folder = f"Integrated_Results/imgs/{dataset_name}/{pipeline_name}/"
    ...
    # Subsampling disabled for fair comparison with FCovSel (uses all samples)
    #X, y = subsample_stratified(X, y, max_samples=15000)
    ...
    plot_spectral("Raw Data", f"all_raw_spectra.jpg", unique_classes_raw, X_df, img_folder)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(img_folder, f"outlier_detection_{pipeline_name}.jpg"), dpi=150)
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(img_folder, f"pca_scores_{pipeline_name}.jpg"), dpi=150)
    plt.close()
    
    # --- Plot 3: Cleaned and All Spectra using Seaborn ---
    unique_classes = X_clean.index.get_level_values('Class').unique()
    plot_spectral(title, f"cleaned_spectra_{pipeline_name}.jpg", unique_classes, X_clean, img_folder)
    
    unique_classes_processed = X_processed.index.get_level_values('Class').unique()
    plot_spectral("", f"all_spectra_{pipeline_name}.jpg", unique_classes_processed, X_processed, img_folder)
    ...
    if skip_to not in ['cars', 'boss', 'ga_ipls', 'ga_ipls_boss']:
        print("\n[Baseline] Running SVM-RBF with ALL features...")
        baseline_path = os.path.join(os.path.abspath(os.getcwd()), "Integrated_Results", pipeline_name, 'Baseline')
        if not os.path.exists(baseline_path):
            os.makedirs(baseline_path)
        
        # Use all wavelengths
        all_wavelengths = [float(c) for c in X_clean.columns]
        run_svm_validation(X_clean, None, all_wavelengths, baseline_path, "Baseline")

    # --- Run the CARS Model ---
    if skip_to not in ['cars', 'boss', 'ga_ipls', 'ga_ipls_boss']:
        print("\nRunning CARS model...")
        start_time = time.time()
        path = os.path.join(os.path.abspath(os.getcwd()), "Integrated_Results", pipeline_name, 'CARS')
        
        # Instantiate WST object first
        ...
        if w_str:
            w_target = [float(i.split('.')[0]) if '.' in i else float(i) for i in w_str]
            run_svm_validation(X_clean, X_clean.index.get_level_values('Class'), w_target, path, "CARS")
    else:
    ...
    if skip_to not in ['boss', 'ga_ipls', 'ga_ipls_boss']:
        print("\nRunning BOSS model...")
        start_time = time.time()
        path = os.path.join(os.path.abspath(os.getcwd()), "Integrated_Results", pipeline_name, 'BOSS')
        ...
        if os.path.exists(os.path.join(path, 'elapsed_time.txt')):
            ...
            c.perform_pca()
            c.save_results()
            c.boss_model(num_bootstrap=500, end=50, speed=0)
            c.load_results_partial()
            save_elapsed_time(start_time, path)
        ...
        if w_str:
            w_target = [float(i.split('.')[0]) if '.' in i else float(i) for i in w_str]
            run_svm_validation(X_clean, X_clean.index.get_level_values('Class'), w_target, path, "BOSS")
    else:
    ...
    if skip_to not in ['ga_ipls', 'ga_ipls_boss']:
        ...
        start_time = time.time()
        path = os.path.join(os.path.abspath(os.getcwd()), "Integrated_Results", pipeline_name, 'GA-iPLS')
        ...
        if os.path.exists(os.path.join(path, 'elapsed_time.txt')):
            ...
            c.perform_pca()
            c.save_results()
            c.compute_ga_ipls(population_size=40, generations=30, crossover_prob=0.6, mutation_prob=0.1, n_intervals=100)
            c.load_results_partial()
            save_elapsed_time(start_time, path)
        ...
        if w_str:
            w_target = [float(i.split('.')[0]) if '.' in i else float(i) for i in w_str]
            run_svm_validation(X_clean, X_clean.index.get_level_values('Class'), w_target, path, "GA-iPLS")
    else:
    ...
    path = os.path.join(os.path.abspath(os.getcwd()), "Integrated_Results", file_name)
    ...
    # Run the BOSS model with the selected wavelengths
    path = os.path.join(os.path.abspath(os.getcwd()), "Integrated_Results", pipeline_name, 'GA-iPLS_BOSS')
    c = WST(path, col_group, X_clean, MAX_COMPONENTS=n_components, CV_FOLD=5, test_percentage=0.3, cutoff=0.5)
    c.perform_pca()
    c.save_results()
    c.boss_model(num_bootstrap=500, end=50, speed=0)
    ...
    if w_str:
        w_target = [float(i.split('.')[0]) if '.' in i else float(i) for i in w_str]
        run_svm_validation(X_clean, X_clean.index.get_level_values('Class'), w_target, path, "GA-iPLS+BOSS")
...
def main():
    ...
    img_folder = "Integrated_Results/imgs"
    if not os.path.exists(img_folder):
        os.makedirs(img_folder)
